...
public class ShadowLayout extends LinearLayout {
    ...
    public ShadowLayout(final Context context, final AttributeSet attrs, final int defStyleAttr) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, com.gigamole.library.R.styleable.ShadowLayout);
        try {
            setIsShadowed(typedArray.getBoolean(com.gigamole.library.R.styleable.ShadowLayout_sl_shadowed, true));
            setShadowRadius(
                    typedArray.getDimension(
                            com.gigamole.library.R.styleable.ShadowLayout_sl_shadow_radius, DEFAULT_SHADOW_RADIUS
                    )
            );
            setShadowDistance(
                    typedArray.getDimension(
                            com.gigamole.library.R.styleable.ShadowLayout_sl_shadow_distance, DEFAULT_SHADOW_DISTANCE
                    )
            );
            setShadowAngle(
                    typedArray.getInteger(
                            com.gigamole.library.R.styleable.ShadowLayout_sl_shadow_angle, (int) DEFAULT_SHADOW_ANGLE
                    )
            );
            setShadowColor(
                    typedArray.getColor(
                            com.gigamole.library.R.styleable.ShadowLayout_sl_shadow_color, DEFAULT_SHADOW_COLOR
                    )
            );
        } finally {
            typedArray.recycle();
