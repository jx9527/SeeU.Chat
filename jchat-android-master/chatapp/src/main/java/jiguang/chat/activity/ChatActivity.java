...
public class    ChatActivity extends BaseActivity implements FuncLayout.OnFuncKeyBoardListener, View.OnClickListener {
    ...
    private void initData() {
        ...
        mPiPeiString = intent.getStringExtra("matchString");
...
//        mChatAdapter.initMediaPlayer();
    ...
    private void initEmoticonsKeyBoardBar() {
       ...
       List<CatchUserEntry> result = new Select().from(CatchUserEntry.class).execute();

       for(int i=0;i<result.size();i++){
           if(result.get(i).getUsername().equals(mTargetId) ){
                   if(result.get(i).getSend() == 0){
                       scrollToBottom();
                       Message msg1;
                       
                       TextContent content1 = new TextContent(mPiPeiString);
                       if (mAtAll) {
                           Log.d("mAtALL执行了", "initEmoticonsKeyBoardBar: ");
                           msg1 = mConv.createSendMessageAtAllMember(content1, null);
                           mAtAll = false;
                       } else if (null != mAtList) {
                           Log.d("null != mAtList执行了", "initEmoticonsKeyBoardBar: ");
                           msg1 = mConv.createSendMessage(content1, mAtList, null);
                       } else {
                           Log.d("真正的执行了", "initEmoticonsKeyBoardBar: ");
                           msg1 = mConv.createSendMessage(content1);
                       }
                       MessageSendingOptions options = new MessageSendingOptions();
                       options.setNeedReadReceipt(true);
                       JMessageClient.sendMessage(msg1, options);
                       mChatAdapter.addMsgFromReceiptToList(msg1);
                       if (mAtList != null) {
                           mAtList.clear();
                       }
                       if (forDel != null) {
                      ...
                      new Update(CatchUserEntry.class).set("send=?",1).where("username=?",mTargetId).execute();
                   }
           }
       }
